    ...
    return {
        ...
        "ingestion": {
            ...
            "steps": [
                ...
                {"step": 4, "name": "Chunking", "description": f"Tekst wordt opgesplitst in chunks van ~{settings.chunk_size} tekens met {settings.chunk_overlap} overlap. Sentence-aware overlap zorgt voor schone breekpunten. PDF-chunks krijgen automatisch paginanummer in metadata"},
                ...
                {"step": 6, "name": "Batch Embedding", "description": f"Verrijkte tekst wordt gevectoriseerd met {settings.embedding_model} via Ollama (768 dimensies). Native batch-embedding (50 teksten per call) voor snellere verwerking"},
        ...
        "stability": {
            ...
            "features": [
                ...
                {"name": "Content Security Policy", "description": "Strikte CSP header beperkt script-bronnen tot 'self' + gepinde CDN's (jsdelivr, cloudflare), blokkeert inline scripts, beperkt connect-src tot 'self', frame-ancestors 'none' — voorkomt XSS payload execution zelfs bij geïnjecteerde HTML"},
        ...
        "config": {
            ...
            "values": {
                ...
                "Embedding Model": f"{settings.embedding_model} (768-dim via Ollama, batch 50)",
